        ...
        <section class="section dashboard">
            <div class="card info-card sales-card">
                <div class="card-body">
                    ...
                    <?php
                        $sql = "SELECT * FROM `guruh` WHERE `GuruhID`='".$_GET['GuruhID']."'";
                        $res = $Confige->SelectAll($sql);
                        $row = $res->fetch_assoc();
                        $sql1 = "SELECT * FROM `guruh_bonus` WHERE `GuruhID`='".$_GET['GuruhID']."'";
                        $res1 = $Confige->SelectAll($sql1);
                        $row1 = $res1->fetch_assoc();
                    ?>
                    <form action="./php/guruh.php?GuruhID=<?php echo $_GET['GuruhID']; ?>" method="post">
                        <div class="row">
                            <div class="col-lg-2"></div>
                            <div class="col-lg-8">
                                <label>Guruh</label>
                                <input type="text" class="form-control mb-2" name="guruhname" value="<?php echo $row['GuruhName']; ?>" required>
                                <label>Boshlanish</label>
                                <input type="text" class="form-control mb-2" name="Start" value="<?php echo $row['Start']; ?>" required>
                                <label>Yakunlanish</label>
                                <input type="text" class="form-control mb-2" name="End" value="<?php echo $row['End']; ?>" required>
                                <label>Xona</label>
                                <input type="text" class="form-control mb-2" value="<?php echo $row['Xona']; ?>" disabled required>
                                <label>Tulov Summa</label>
                                <input type="number" class="form-control mb-2" name="summa" value="<?php echo $row['Summa']; ?>" required>
                                <label>O'qituvchiga to'lov</label>
                                <input type="number" class="form-control mb-2" name="TulovTech" value="<?php echo $row1['Tulov']; ?>" required>
                                <label>O'qituvchiga bonus</label>
                                <input type="number" name="TulovBonus" class="form-control mb-3" value="<?php echo $row1['Bonus']; ?>" required>
                                <button type='submit' name="guruhEdit" class='btn btn-primary w-100'>Saqlash</button>
                            </div>
                        </div>
                    </form>
                </div>
